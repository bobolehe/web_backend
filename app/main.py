    ...
    try:
        ...
        for project in completed_projects:
            try:
                ...
                # 启动服务器（带反向代理）
                # 规范化 source_url 为仅域名部分，避免代理目标包含路径/查询参数
                proxy_url = None
                ...
                logger.error(
                    f"Error restarting server for project {project.id}: {str(e)}"
                )
                continue
...
if __name__ == "__main__":
    ...
    uvicorn.run("app.main:app", host="0.0.0.0", port=8845, reload=settings.debug)
