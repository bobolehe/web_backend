    ...
    def clone_website(cls, db: Session, project_id: str) -> bool:
        ...
        if not project.source_url:
            ...
            ProjectService.update_project_status(
                db, project_id, ProjectStatus.FAILED, error_message="未设置源网址"
            )
            return False
        ...
        try:
            ...
            logger.info(
                f"Starting to clone {project.source_url} for project {project_id}"
            )
            ...
            # 替换 CDN 域名的绝对 URL 为本地相对路径
            # 这对于 SPA 页面特别重要，例如 Apifox 的 locale 脚本从 cdn.apifox.com 加载
            cdn_patterns = [
                r"https?://cdn\.apifox\.com/",
                r"https?://cdn\.apifox\.cn/",
            ]
            for cdn_pattern in cdn_patterns:
                html_content = re.sub(cdn_pattern, "/", html_content)

            # 同时替换源站域名的绝对 URL 为相对路径
            html_content = html_content.replace(f"{base_url}/", "/")
            html_content = html_content.replace(f"{base_url}", "/")

            logger.info(f"Replaced CDN and source URLs for project {project_id}")
...
"""
            ...
            logger.info(
                f"Saved HTML to {html_file}, downloaded {len(downloaded_resources)} resources"
            )
            ...
            # 启动 HTTP 服务器（带反向代理）
            # 使用 base_url（仅域名）而非完整 source_url，避免代理目标包含路径/查询参数
            port = ServerManager.start_server(
                project_id,
                project_dir,
                preferred_port,
                source_url=base_url,  # 仅使用域名部分作为代理目标
            )
            ...
            if port:
                ...
                ProjectService.update_project_status(
                    db, project_id, ProjectStatus.COMPLETED, content_path=content_path
                )
            ...
            ProjectService.update_project_status(
                db, project_id, ProjectStatus.FAILED, error_message=error_msg
            )
            return False

    @classmethod
    def start_proxy_mode(cls, db: Session, project_id: str):
        """
        代理模式：不使用 Playwright 克隆，直接启动缓存式反向代理服务器。
        所有资源在用户首次访问时自动从源站获取并缓存到本地。
        """
        project = ProjectService.get_project(db, project_id)
        if not project:
            logger.error(f"Project {project_id} not found")
        ...
        if not project.source_url:
            logger.error(f"Project {project_id} has no source URL")
            ProjectService.update_project_status(
                db, project_id, ProjectStatus.FAILED, error_message="未设置源网址"
            )
            return False
        ...
        try:
            ...
            if port:
                content_path = f"/cloned/{project_id}/"
                project.port = str(port)
                ProjectService.update_project_status(
                    db, project_id, ProjectStatus.COMPLETED, content_path=content_path
                )
                logger.info(
                    f"Proxy mode started for {project.source_url}, "
                    f"accessible at http://127.0.0.1:{port}"
                )
            else:
                ProjectService.update_project_status(
                ...
                logger.warning(
                    f"Proxy mode for {project.source_url} but failed to start server"
                )

            return True

        except Exception as e:
            error_msg = f"代理模式启动失败: {str(e)}"
            logger.error(
                f"Error starting proxy mode for {project.source_url}: {error_msg}"
            )
            import traceback
