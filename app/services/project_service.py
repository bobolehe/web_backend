...
class ProjectService:
    ...
    def create_project(
        ...
        Returns:
            创建的项目对象
        """
        from app.models.project import CloneMode

        if clone_mode is None:
            clone_mode = CloneMode.STATIC
        ...
        logger.info(
            f"Project '{name}' created with ID: {project.id}, mode: {clone_mode}"
        )
        return project
    ...
    def update_project(
        ...
        if source_url_changed:
            ...
            logger.info(
                f"Project '{project.name}' source URL changed, status reset to PENDING"
            )
