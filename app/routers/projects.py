...
from app.models import ProjectStatus, CloneMode
...
async def create_project(
    ...
    project = ProjectService.create_project(
        ...
        clone_mode=project_data.clone_mode,
    )

    logger.info(
        f"User '{current_user.username}' created project '{project.name}' (mode: {project.clone_mode})"
    )
...
async def clone_project(
    ...
    if not project.source_url:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="项目未设置源网址，无法克隆"
        )

    # 根据克隆模式添加后台任务
    if project.clone_mode == CloneMode.PROXY:
        # 代理模式：直接启动缓存式反向代理服务器
        background_tasks.add_task(CloneService.start_proxy_mode, db, project_id)
        logger.info(
            f"User '{current_user.username}' started proxy mode for project '{project.name}'"
        )
        return {"message": "代理模式启动中", "project_id": project_id}
    else:
        # 静态模式：使用 Playwright 克隆
        background_tasks.add_task(CloneService.clone_website, db, project_id)
        logger.info(
            f"User '{current_user.username}' started cloning project '{project.name}'"
        )
        return {"message": "克隆任务已启动", "project_id": project_id}
...
async def restart_server(
    ...
    logger.info(
        f"User '{current_user.username}' restarted (re-clone) project '{project.name}'"
    )

    return {"message": "重新克隆任务已启动", "project_id": project_id}
...
async def start_server(
    ...
    if project.status != ProjectStatus.COMPLETED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="只能启动已完成克隆的项目"
        )
    ...
    if not project_dir.exists():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="项目克隆目录不存在"
        )
    ...
    port = ServerManager.start_server(
        ...
        project_dir,
        preferred_port,
        source_url=proxy_url,  # 仅使用域名部分作为代理目标
    )
    ...
    if port:
        ...
        logger.info(
            f"User '{current_user.username}' started server for project '{project.name}' on port {port}"
        )
...
async def stop_server(
    ...
    server_info = ServerManager.get_server_info(project_id)
    if not server_info:
        return {"message": "服务器未运行", "project_id": project_id}
    ...
    if success:
        logger.info(
            f"User '{current_user.username}' stopped server for project '{project.name}'"
        )
        return {"message": "服务器已停止", "project_id": project_id}
    else:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="停止服务器失败"
        )
